...
///
/// This class is overkill when it comes to reporting on iCloud sync. Normally, `NSPersistentCloudKitContainer` will
/// sync happily and you can
/// leave it alone. Every once in a while, however, it will hit an error that makes it stop syncing. This is what you
/// really want to detect, because, since iCloud
...
///     }
///
/// `syncError` and `notSyncing`, together, tell you if there's a problem that `NSPersistentCloudKitContainer` has
/// announced or not announced
/// (respectively).
/// The `setupError`, `importError`, and `exportError` properties can give you the reported error. Digging deeper,
/// `setupState`, `importState`,
/// and `exportState` give you the state of each type of `NSPersistentCloudKitContainer` event in a nice little
/// `SyncState` enum with associated
/// values that let you get even more granular, e.g. to find whether each type of event is in progress, succeeded, or
/// failed,  the start and end time of the event, and
...
public class SyncMonitor: ObservableObject {
    ...
    /// - Otherwise, if the iCloud account isn't available (e.g. they're not logged in or have disabled iCloud for the
    /// app in Settings or System Preferences), the
    ///     state summary is`.accountNotAvailable`.
    /// - Otherwise, if `NSPersistentCloudKitContainer` reported an error for any event type the last time that event
    /// type ran, the state summary is
    ///     `.error`.
    ...
    public var syncStateSummary: SyncSummaryStatus {
        if self.networkAvailable == false { return .noNetwork }
        guard case .available = self.iCloudAccountStatus else { return .accountNotAvailable }
        if self.syncError { return .error }
        if self.notSyncing { return .notSyncing }
        if case .notStarted = self.importState, case .notStarted = self.exportState,
           case .notStarted = self.setupState {
            return .notStarted
        }

        if case .inProgress = self.setupState { return .inProgress }
        if case .inProgress = self.importState { return .inProgress }
        if case .inProgress = self.exportState { return .inProgress }

        if case .succeeded = self.importState, case .succeeded = self.exportState {
            return .succeeded
        }
        return .unknown
    }
    ...
    /// If `syncError` is true, then either `setupError`, `importError` or `exportError` (or any combination of them))
    ...
    public var syncError: Bool {
        self.setupError != nil || self.importError != nil || self.exportError != nil
    }

    /// Returns `true` if there's no reason that we know of why sync shouldn't be working
    ///
    /// That is, the user's iCloud account status is "available", the network is available, there are no recorded sync
    /// errors, and setup is complete and succeeded.
    public var shouldBeSyncing: Bool {
        if case .available = self.iCloudAccountStatus, self.networkAvailable == true, !self.syncError,
           case .succeeded = self.setupState {
            return true
        }
        return false
    }
    ...
    ///
    /// `notSyncing` is true if `shouldBeSyncing` is true (see `shouldBeSyncing`) but `importState` is still
    /// `.notStarted`.
    ///
    /// The first thing `NSPersistentCloudKitContainer`does when the app starts is to set up, then run an import. So,
    /// `notSyncing` should be true for
    /// a very very short period of time (e.g. less than a second) for the time between when setup completes and the
    /// import starts. As such, it's suitable for
    /// displaying an error graphic to the user, e.g. `Image(systemName: "xmark.icloud")` if `notSyncing` is `true`, but
    /// not necessarily for
    ...
    ///     }
    ///
    /// I would argue that `notSyncing` being `true` for a longer period of time indicates a bug in
    /// `NSPersistentCloudKitContainer`. E.g. the case
    /// that made me write this computed property is that if Settings on iOS wants the user to log in again, CloudKit
    /// will report a "partial error" when setting up,
    /// but ultimately send a notifiation stating that setup was successful; however, CloudKit will then just not sync,
    /// providing no errors. `notSyncing` detects
    /// this condition, and those like it. If you see `notSyncing` being triggered, I'd recommend isolating the issue
    /// (e.g. the one above) and filing a FB about it
    /// to Apple.
    public var notSyncing: Bool {
        if case .notStarted = self.importState, self.shouldBeSyncing {
            return true
        }
        return false
    }
    ...
    ///
    /// This means `NSPersistentCloudKitContainer` probably won't try to do imports or exports, which means that data
    /// won't be synced. However, it's
    /// usually caused by something that can be fixed without deleting the DB, so it usually means that sync will just
    /// be delayed, unlike exportError, which
    /// usually requires deleting the local DB, thus losing changes.
    ///
    /// You should examime the error for the cause. You may then be able to at least report it to the user, if not
    /// automate a "fix" in your app.
    public var setupError: Error? {
        if self.networkAvailable == true, let error = setupState.error {
            return error
        }
        return nil
    }
    ...
    public var importError: Error? {
        if self.networkAvailable == true, let error = importState.error {
            return error
        }
        return nil
    }
    ...
    ///     }
    ///
    /// This method is the main reason this module exists. When NSPersistentCloudKitContainer "stops working", it's
    /// because it's hit an error from which it
    /// can not recover. If that error happens during an export, it means your user's probably going to lose any changes
    /// they make (since iCloud is the
    /// "source of truth", and NSPersistentCloudKitContainer can't get their changes to iCloud).
    /// The key to data safety, then, is to detect and correct the error immediately. `exportError` is designed to
    /// detect this unrecoverable error state
    /// the moment it happens. It specifically tests that the network is available and that an error was reported
    /// (including error text). This means that sync
    /// _should_ be working (that is, they're online), but failed. The user, or your application, will likely need to
    /// take action to correct the problem.
    public var exportError: Error? {
        if self.networkAvailable == true, let error = exportState.error {
            return error
        }
        return nil
    }
    ...
    ///
    /// This is true if the network is available in any capacity (Wi-Fi, Ethernet, cellular, carrier pidgeon, etc) - we
    /// just care if we can reach iCloud.
    ...
    ///
    /// This can be helpful in diagnosing "notSyncing" issues or other "partial error"s from which CloudKit thinks it
    /// recovered, but didn't really.
    public private(set) var lastError: Error?

    // MARK: - Listeners -

    /// Where we store Combine cancellables for publishers we're listening to, e.g. NSPersistentCloudKitContainer's
    /// notifications.
    ...
    public init(setupState: SyncState = .notStarted, importState: SyncState = .notStarted,
        ...
        self.monitor.pathUpdateHandler = { path in
            DispatchQueue.main.async {
                #if os(watchOS)
                    self.networkAvailable = (!path.availableInterfaces.isEmpty)
                #else
    ...
    ///
    /// When SyncMonitor is listening to notifications (which it does unless you tell it not to when initializing), this
    /// method is called each time CKContainer
    ...
    /// The state of a CloudKit import, export, or setup event as reported by an `NSPersistentCloudKitContainer`
    /// notification
    public enum SyncState {
        ...
        /// The last sync of this type finished and succeeded (`succeeded` was `true` in the notification from
        /// `NSPersistentCloudKitContainer`).
        case succeeded(started: Date, ended: Date)

        /// The last sync of this type finished but failed (`succeeded` was `false` in the notification from
        /// `NSPersistentCloudKitContainer`).
        ...
        ///
        /// This is the main property you'll want to use to detect an error, as it will be `nil` if the sync is
        /// incomplete or succeeded, and will contain
